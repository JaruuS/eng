...
def prepare_dataset():
    ...
    k = 0
    for i in range(1, 33):
        x = pickle.load(open('D:\eng\data_preprocessed_python\s' + str(i).zfill(2) + '.dat', 'rb'), encoding='latin1')
        # 37-GSR 39-Plet
        data = x['data']
        gsr = data[:, 36, ::32]
        plet = data[:, 38, ::32]
...
# x = numpy.loadtxt(open('D:\\eng\\data_preprocessed_python\dataset.txt', 'rb'))
# print(x.shape)
prepare_dataset()
x = split_dataset_into_training_and_test(numpy.loadtxt(open('D:\\eng\\data_preprocessed_python\\dataset.txt', 'rb')), numpy.loadtxt(open('D:\\eng\\data_preprocessed_python\\hot_vectors.txt', 'rb')))
with open('D:\\eng\\data_preprocessed_python\\train_dataset.txt', 'wb') as outfile:
    numpy.savetxt(outfile, x[0], "%.2f")
with open('D:\\eng\\data_preprocessed_python\\train_labels.txt', 'wb') as outfile:
    numpy.savetxt(outfile, x[1], "%.0f")
with open('D:\\eng\\data_preprocessed_python\\test_dataset.txt', 'wb') as outfile:
    numpy.savetxt(outfile, x[2], "%.2f")
with open('D:\\eng\\data_preprocessed_python\\test_labels.txt', 'wb') as outfile:
    numpy.savetxt(outfile, x[3], "%.0f")
